...
const handleImport = async () => {
  ...
  try {
    const response = await importRoles(selectedFile.value);
...
const downloadRoleSample = () => {
  const content = [
    "name,guard_name,permissions",
    'Admin,web,"dashboard.view,user.view,user.create,user.update,user.delete"',
    'Manager,web,"dashboard.view,user.view,role.view"',
    'Viewer,web,"dashboard.view,user.view"',
  ].join("\n");
  ...
  downloadBlobFile(blob, "roles-sample.csv");
};
</script>
...
<template>
  <AppModal
    :open="open"
    title="Import Roles"
    subtitle="Upload a CSV file to create or update roles and assign permissions."
    size="md"
    @close="handleClose"
  >
    <template #icon>
    ...
    <div class="space-y-5">
      ...
      <AlertMessage type="info" title="CSV Format">
        Your CSV must include <strong>name</strong>. The <strong>guard_name</strong> and
        <strong>permissions</strong> columns are optional. Permissions should be
        comma-separated.
      </AlertMessage>
...
Admin,web,"dashboard.view,user.view,user.create,user.update,user.delete"
Manager,web,"dashboard.view,user.view,role.view"
Viewer,web,"dashboard.view,user.view"</pre
        >
      </div>
      ...
      >
        ...
        <AppButton type="button" variant="secondary" @click="downloadRoleSample">
          Download Sample
        </AppButton>
      </div>
    </div>
